...
def create_prism_gwl(x1=0,x2=100,y1=0,y2=10,h=5,t1=m.radians(40),t2=m.radians(50),fill_h=0.3,cont_n=0,slice=0.1,
                                                                             cont_h=0.1,fileN='xx',fileP=''):
    ns_path = 'C:\Users\Avi Braun\Dropbox\LAB B014\NaNoscribe\Structures\Prism'
    name_of_file='30-40-1'+fileN+'_.gwl'
    ...
    for i in range(0, int(slice_n)):
        ...
        y_start_f =y_start+(cont_n-0.5)*cont_h
        y_end_f = y_end-(cont_n-0.5)*cont_h
        fill_space(cont_n, cont_h, slice, fill_h, x_start, x_end, y_start=y_start_f, y_end=y_end_f, dx_start=dx_start,
...
def fill_space(cont_n,cont_h,slice,fill_h,x_start,x_end,y_start,y_end,dx_start,dx_end,file_handle):
    ...
    fill_n = int((x_end - x_start - (cont_n - 1) * cont_h * 2) / fill_h)
    ...
    for i in range(0, int(fill_n)):
        x1 = x1 + fill_h
        xy = np.array([[x1, y_start, 0], [x1, y_end, 0]])
...
create_prism_gwl(x1=0, x2=100, y1=0, y2=50, h=40, t1=m.radians(40), t2=m.radians(48), fill_h=0.3, cont_n=0, slice=0.1, cont_h=0.1, fileN='test_short', fileP='')
